...
%
% [비교군: outer/dark는 local modal, bright 내부는 4개 pressure point + 균일 CMP 출력 제약 버전]
%
% 목적:
%   1) 스피커 위치 선택은 시뮬레이션 모델 기반으로 먼저 수행
%   2) 목적함수는 outer/dark는 local modal coefficient matching, bright 내부는 4개 pressure point matching 사용
...
%
% 핵심 구조:
%   - bright outer/dark boundary pressure -> local modal coefficient beta
%   - bright inner region: 4 uniformly placed pressure control/test points
%   - selection: multi-frequency hybrid LS (outer modal + inner pressure + dark modal)
%   - final q(f): same hybrid-domain LS + uniform CMP power constraint
%
% 필요한 외부 qSafe 파일 없음.
...
brightInnerPressurePts = [bright.center(1) + bright.radius_inner*cos(th), ...
                  bright.center(2) + bright.radius_inner*sin(th)];

% 비교군 설정: 기존 inner boundary local modal 대신, bright zone 내부 4개 pressure point 사용
% 4점은 bright center 기준 반지름 bright.radius_inner 위의 사분면 방향에 균등 배치한다.
innerPressureAngles = deg2rad([45; 135; 225; 315]);
brightInnerPressurePts = [bright.center(1) + bright.radius_inner*cos(innerPressureAngles), ...
                          bright.center(2) + bright.radius_inner*sin(innerPressureAngles)];
N_inner_pressure = size(brightInnerPressurePts, 1);
...
% outer/dark는 local modal coefficient matching 유지.
% bright 내부는 local modal coefficient가 아니라 4개 pressure point를 직접 matching한다.
% 따라서 Mmax_inner는 비교군 코드에서는 사용하지 않고, NumInnerPressurePts 열은 내부 pressure point 개수를 의미한다.
% outer/dark의 실제 사용 차수/인덱스는 주파수별로 달라진다.
Mmax_outer = 4;
Mmax_inner = 2;  % 비교군에서는 사용하지 않음: inner는 4 pressure points
Mmax_dark  = 4;
...
for kk = 1:K
    ...
    % outer/dark는 local modal order를 주파수별로 선택한다.
    % bright 내부는 local modal로 변환하지 않고, 4개 pressure point를 그대로 사용한다.
    m_idx_inner = (1:N_inner_pressure).';  % mode index가 아니라 inner pressure point index
    ...
    F_inner = eye(N_inner_pressure);  % inner pressure point matching: beta_inner = d_inner
    ...
    d_inner_raw = point_source_sum(brightInnerPressurePts, virtual_source_L, virtual_source_R, k);
    ...
    for l = 1:Nc
        ...
        h_inner_l = reflected_directional_transfer(brightInnerPressurePts, candidates(l), k, spkModel, room);
...
%% [11] 전 대역 공통 스피커 선택: outer/dark modal + inner pressure objective
fprintf('=== 전 대역 공통 스피커 selection 시작: hybrid-domain ===\n');
...
%% [12] 주파수별 final q 계산: hybrid-domain + uniform CMP constraint
q_all = cell(K,1);
...
for kk = 1:K
    ...
    H_inner = zeros(size(brightInnerPressurePts,1), nSelect);
    ...
    for i = 1:nSelect
        ...
        H_inner(:,i) = reflected_directional_transfer(brightInnerPressurePts, candidates(selected(i)), k, spkModel, room);
    ...
    if any(repIdx == kk)
        ...
        repTables.(fieldName).inner = build_pressure_point_table('inner_pressure', modalInfo{kk}.m_idx_inner, beta_inner, beta_inner_rec);
...
summaryTbl = table( ...
    ...
    weightedEnergyUsage, ...
    'VariableNames', ...
    {'FreqHz','NumModesOuter','NumInnerPressurePts','NumModesDark', ...
     'ModalNMSE_Outer_dB','InnerPointNMSE_dB','ModalResidual_Dark_dB', ...
...
modalOrderSummaryTbl = table(fList(:), numModes_outer, numModes_inner, numModes_dark, ...
    'VariableNames', {'FreqHz','NumModesOuter','NumInnerPressurePts','NumModesDark'});

%% [15] 결과 출력
fprintf('\n====================================================\n');
fprintf('Outer/Dark local modal + Inner 4 pressure points + ceil(kR) adaptive order + 균일 CMP 출력 제약 결과\n');
...
fprintf('\n================ 주파수별 adaptive modal mode 개수 및 inner pressure point 개수 앞 20행 ================\n');
...
plot(brightInnerPressurePts(:,1), brightInnerPressurePts(:,2), 'c.', 'MarkerSize', 10);
...
legend('Outer modal NMSE','Inner pressure-point NMSE','Dark modal residual','Location','best');
...
H_inner_rep = zeros(size(brightInnerPressurePts,1), nSelect);
...
for i = 1:nSelect
    ...
    H_inner_rep(:,i) = reflected_directional_transfer(brightInnerPressurePts, candidates(selected(i)), k_rep, spkModel_rep, room);
...
theta_deg = th*180/pi;
innerPointIdx = 1:length(d_inner_rep);
...
nexttile;
plot(innerPointIdx, abs(d_inner_rep), 'ko-', 'LineWidth', 2); hold on;
plot(innerPointIdx, abs(P_inner_rep), 'rs--', 'LineWidth', 1.8);
grid on; xlabel('Inner pressure point index'); ylabel('Magnitude');
title('Inner pressure-point magnitude'); legend('Target','Reconstructed','Location','best');

nexttile;
plot(innerPointIdx, unwrap(angle(d_inner_rep)), 'ko-', 'LineWidth', 2); hold on;
plot(innerPointIdx, unwrap(angle(P_inner_rep)), 'rs--', 'LineWidth', 1.8);
grid on; xlabel('Inner pressure point index'); ylabel('Phase (rad)');
title('Inner pressure-point phase'); legend('Target','Reconstructed','Location','best');
...
nexttile;
plot(innerPointIdx, phaseErr_inner_rep, 'bo-', 'LineWidth', 1.8); hold on;
yline(0, 'k--', 'LineWidth', 1.0);
yline(pi, 'r:', 'LineWidth', 1.0);
yline(-pi, 'r:', 'LineWidth', 1.0);
grid on; xlabel('Inner pressure point index'); ylabel('Phase error (rad)');
ylim([-pi pi]);
title(sprintf('Inner pressure-point wrapped phase error @ %d Hz', f_rep));
legend('angle(P_{rec}/P_{target})','0','\pm\pi','Location','best');
hold off;
...
gainScaleSummaryTbl = table( ...
    ["Outer modal boundary NMSE"; "Inner pressure-point NMSE"; "Dark energy"; "Bright/Dark contrast"; ...
...
nexttile;
plot(innerPointIdx, abs(d_inner_rep), 'ko-', 'LineWidth', 2); hold on;
plot(innerPointIdx, abs(P_inner_rep), 'rs--', 'LineWidth', 1.6);
plot(innerPointIdx, abs(P_inner_gain_rep), 'bd-.', 'LineWidth', 1.8);
grid on; xlabel('Inner pressure point index'); ylabel('Magnitude');
title('Inner pressure-point magnitude: raw vs gain-scaled');
legend('Target','Raw recon','Gain-scaled recon','Location','best');
hold off;
...
%% [22] 결과 저장
save('outermodal_inner4pressure_adaptive_ceil_uniform_cmp_gaincheck_multirep_2000_result.mat', ...
...
fprintf('\n결과 저장 완료: outermodal_inner4pressure_adaptive_ceil_uniform_cmp_gaincheck_multirep_2000_result.mat\n');
...
function T = build_pressure_point_table(zoneName, point_idx, targetPressure, reconPressure)
    N = length(point_idx);

    targetReal = real(targetPressure(:));
    reconReal  = real(reconPressure(:));
    realDiff   = reconReal - targetReal;

    targetImag = imag(targetPressure(:));
    reconImag  = imag(reconPressure(:));
    imagDiff   = reconImag - targetImag;

    targetMag  = abs(targetPressure(:));
    reconMag   = abs(reconPressure(:));
    absErr     = abs(reconPressure(:) - targetPressure(:));
    ...
    T = table( ...
        ...
        absErr, ...
        'VariableNames', ...
        {'Zone','PointIndex','TargetReal','ReconReal','RealDiff', ...
